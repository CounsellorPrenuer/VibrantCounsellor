...
export function ContactSection() {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    // 1. Attempt to save lead in background (don't await strictly to prevent delay)
    saveLead(values);

    try {
      // 2. Prepare and trigger mailto
      const subject = encodeURIComponent(`Inquiry: ${values.subject}`);
      ...
      toast({
        ...
        description: "Your message has been captured. Opening your email client now.",
      });

      // Cross-browser reliable redirect
      const mailWindow = window.open(mailtoLink, "_self");
      
      // Fallback if _self is blocked or fails
      if (!mailWindow) {
      ...
      toast({
        ...
        description: "Please manually email us at " + (settings?.contactEmail || "counselor963@gmail.com"),
      });
    }
  };
